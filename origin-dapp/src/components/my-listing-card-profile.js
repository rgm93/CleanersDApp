...
class MyListingCard extends Component {
  constructor(props) {
    ...
    this.intlMessages = defineMessages({
      confirmCloseListing: {
        id: 'my-listing-card-profile.confirmCloseListing',
        defaultMessage:
          'Are you sure that you want to permanently close this listing? This cannot be undone.'
      },
      confirmOpenListing: {
        id: 'my-listing-card-profile.confirmOpenListing',
        defaultMessage:
          'Are you sure that you want to active this listing? This cannot be undone.'
      }
    })
  ...
  async openListing() {
    ...
    const { address } = listing
    const prompt = confirm(
      intl.formatMessage(this.intlMessages.confirmOpenListing)
    )
    ...
    try {
      ...
      const {
        created,
        transactionReceipt
      } = await origin.marketplace.withoutdrawListing(
        this.props.listing.id,
        {},
        (confirmationCount, transactionReceipt) => {
          // Having a transaction receipt doesn't guarantee that the listing status will have changed.
      ...
      this.props.upsertTransaction({
        ...transactionReceipt,
        created,
        transactionTypeKey: 'openListing'
      })
        ...
        <div className="card-body d-flex flex-column flex-lg-row">
          ...
          <div className="content-container d-flex flex-column">
            ...
            {/*<p className="price">
              ...
              {!parseInt(unitsRemaining) &&
                <span className="badge badge-info">
                  <FormattedMessage
                    id={ 'my-listing-card-profile.soldOut' }
            ...
            <div className="d-flex counts">
              {/*<p>
                <FormattedMessage
                  id={ 'my-listing-card-profile.totalQuantity' }
            ...
            <div className="actions d-flex">
              <div className="links-container">
                ...
                {status === 'inactive' ? null : (
                  <Fragment>
                    <Link to={`/update/${listing.id}`}>
                        <FormattedMessage
                          id={'my-listing-card-profile.editListing'}
                          defaultMessage={'Editar propiedad'}
                        />
                    </Link>
                    <a className="warning" onClick={this.closeListing}>
                      <FormattedMessage
                        id={'my-listing-card-profile.closeListing'}
                        defaultMessage={'Desactivar propiedad'}
                      />
                    </a>
                  </Fragment>
                )}
                {status === 'active' ? null : (
                  <Fragment>
                    <a className="secondary" onClick={this.openListing}>
                      <FormattedMessage
                        id={'my-listing-card-profile.openListing'}
                        defaultMessage={'Activar propiedad'}
                      />
                    </a>
                  </Fragment>
                )}
